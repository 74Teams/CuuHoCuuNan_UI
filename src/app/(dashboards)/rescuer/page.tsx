...
const MiniMap = dynamic(
  () => import("@/components/dashboards/rescuer/MiniMap"),
  { ssr: false },
);

const NavigateMap = dynamic(() => import("@/components/shared/NavigateMap"), {
  ssr: false,
  loading: () => (
...
export default function RescuerDashboardPage() {
  const { data, isLoading, isError } = useRescuerDashboard();
  ...
  return <RescuerDashboardContent data={data} />;
}

function RescuerDashboardContent({ data }: { data: RescuerDashboardData }) {
  const updateMission = useUpdateRescuerMissionStatusMutation();
  ...
  function handleAcceptMission() {
    ...
    if (!missionId) {
      toast.error("Không có nhiệm vụ đang hoạt động");
      return;
    }
    updateMission.mutate(
      { missionId, status: "EN_ROUTE", note: "Đội đã tiếp nhận" },
      {
        onSuccess: async () => {
      ...
      >
        ...
        {notification.type === "NEW_MISSION_ASSIGNED" && (
          <div className="flex gap-2 mt-2">
            <Button size="sm" className="flex-1" onClick={handleAcceptMission}>
              Tiếp nhận
            </Button>
            <Button
              size="sm"
  ...
  const teamName = data.team?.name ?? "Đội cứu hộ";
  const leaderName = data.team?.leaderName ?? "Trưởng nhóm";
  const missionId = data.activeMission?.id
      ...
      <aside className="hidden border-r bg-card md:flex md:w-[260px] flex-col">
        ...
        <div className="p-4 border-t mt-auto">
          ...
          <button
            type="button"
            onClick={logout}
            className="flex items-center gap-3 w-full rounded-md px-3 py-2 text-sm font-medium text-destructive hover:bg-destructive/10 transition-colors mt-1"
          >
                            ...
                            <div className="flex items-center gap-4 bg-muted/50 p-4 rounded-lg border">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-base font-semibold truncate text-foreground">
                                  {mainVictim.fullName}{" "}
                                  {mainVictim.age && `(${mainVictim.age}T)`}
                                </p>
                  ...
                  <Card className="flex flex-col items-center justify-center h-full min-h-[400px] border-dashed shadow-none">
                    <CardContent className="flex flex-col items-center text-center p-6">
                      ...
                      <p className="text-sm text-muted-foreground max-w-[300px]">
                        Hệ thống đang theo dõi. Đảm bảo trạng thái{" "}
                      ...
                      <div className="flex flex-col">
                        {visibleNotifications.length === 0 ? (
                          ...
                          visibleNotifications.slice(0, 4).map((noti) => (
                            <div
                              key={noti.id}
                              className="p-4 border-b last:border-0 hover:bg-muted/50 transition-colors flex gap-3"
                            >
                              <div
                                className={cn(
                                  "size-8 rounded-full flex items-center justify-center shrink-0",
                                  getNotificationDetails(noti.type).bg,
                                )}
                              >
